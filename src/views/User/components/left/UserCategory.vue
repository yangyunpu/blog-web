...
onMounted: {
  // 加载参数
  loadTagsData = async () => {
    const res = await getUserTagList(param);
    if (res.code == 0) {
      tagsData.value = res.result.list;
      // loading.value = false;
    } else {
      // loading.value = false;
...
<template>
  <div>
    ...
    <el-card shadow="hover" class="collapse">
      ...
      <!-- 新增标签 -->
      <div class="center">
        <el-input v-model="tag_name" placeholder="请输入标签名称" class="input" />
      ...
      <div v-for="(tag, index) in tagsData" style="margin-top: 10px;">
        <div class="center">
          ...
          <el-tag class="tag">{{ tag.tag_name }}</el-tag>
          <div class="flex-grow" />
          <!-- 删除与编辑 -->
          <el-button style="margin-left: 5px;" @click="handleDelete(tag.id)" text>删除</el-button>
          <el-button @click="openEdit(tag.id)" text>编辑</el-button>
        </div><!-- 编辑框 -->
        <div v-if="edit.status && tag.id === edit.tag" style="margin-top: 10px;">
          <div class="center">
            <el-input v-model="editedTag.tag_name" placeholder="请输入新的标签名称" class="input" />
            <el-button type="primary" @click="handleSubmit(tag.id)">确定</el-button>
          </div>
        </div>
      </div>
    </el-card>
